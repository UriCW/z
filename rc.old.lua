...
-- {{{ Help notification
-- Try and open $HOME/.config/awesome/keys.txt and create a help notification
local help_file=io.open(os.getenv('HOME')..'/.config/awesome/keys.txt','r')
local help_msg=""
if (help_file) then
    help_msg=help_file:read("*a")
    help_file:close()
else
    help_msg="Missing help file"
end
local help_notification=nil;

function toggle_help()
        if (help_notification) then 
            naughty.destroy(help_notification)
            help_notification=nil
        else  help_notification=naughty.notify({text=help_msg, timeout=0}) end
end
--toggle_help()
-- }}}
...
terminal = "urxvt"
utils_path="/home/volcan/utils/"
editor = os.getenv("EDITOR") or "vi"
editor_cmd = terminal .. " -e " .. editor
screenshot_cmd = utils_path.."scrshot.sh"
-- Default modkey.
...
-- {{{ Copy and paste using the z.ic module
local ic = z.ic.clipboard({})
ic.lpanel:show()
--- }}}
...
myawesomemenu = {
   ...
   { "keys", function() toggle_help() end },
   { "edit awesome testing", terminal .. " -e ~/Development/awesome/testing2/tmux_env.sh"},
--   { "edit config", editor_cmd .. " " .. awesome.conffile..";"..terminal.."-e xbill" },
...
globalkeys = awful.util.table.join(
    awful.key({ modkey,           }, "F1",  function()  toggle_help() end ),
    ...
    awful.key({                   }, "Print" , function() awful.util.spawn(screenshot_cmd,false) end),
    -- Utils
    --awful.key({ modkey,           }, "k", function() z.utils.ror("konsole") end),
    --awful.key({ modkey,           }, "k", function() z.utils.ror("xbill") end),
